...
- The blurb is collected: the `.content-heading-txt` paragraph after the tagline `<h2>`, tolerant of an unclosed `<p>`. The truncated `<meta name="description">` is not used. Every format's snapshot carries the blurb as its Release Description at authoritative rank (fixture `little-witch-academia-3.html`, fetched 2026-09-26). Existing observations pick it up on their next due refetch. Cover images, contributor roles, page counts, and age ratings are still not collected.
