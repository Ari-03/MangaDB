...
2. A malformed HTTP-200 JSON response, such as an error object without `data.titles`, became a valid empty full sweep. The parser now throws unless a titles array exists. An action regression first imports a manga, injects an error response, and checks that its observation remains active.
